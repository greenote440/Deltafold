...
\paragraph{Ranks 2 and 3 --- SSEs and protein.} Residues receive a DSSP-style
...
\begin{figure}[h]
\centering
\fbox{\parbox[c][3.4cm][c]{0.9\textwidth}{\centering\itshape
visualisation placeholder --- lifting one structure into the PCC: C$\alpha$ trace
$\to$ $k$-NN contact graph (rank 1) $\to$ DSSP segments (rank 2) $\to$ protein cell
(rank 3), with the scalar feature carried by each rank and the displacement vector
on the edges.}}
\caption{[to be added] Featurization of one protein into its Protein Combinatorial Complex.}
\end{figure}
...
to the other. Each cluster is then subsampled at the common rate $f$, preserving the
